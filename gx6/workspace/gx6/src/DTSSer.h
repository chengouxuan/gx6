...
public:
    ...
    typedef TranspositionTable <Item, 1048583> TransTableSuper;
    class TransTable: public TransTableSuper { // 置换表
    	typedef TransTableSuper Super;
    public:
        void Enter(bool isDTSSSuccess, const DMove &dm = DMove()) {
        	Super::Enter(Item(isDTSSSuccess ? Item::TYPE_SUCCESS : Item::TYPE_FAIL, ::CheckSum(), dm), ::Hash());
        }
        bool LookUp(bool &isDTSSSuccess, DMove &dm) {
            Item item;
            if(!Super::LookUp(::CheckSum(), ::Hash(), item) || item._type == Item::TYPE_NOT_A_ITEM) {
                return false;
            }
            dm = item._dMove;
